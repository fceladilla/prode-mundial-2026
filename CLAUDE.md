...
**Free-tier data is delayed and inconsistently cached.** football-data.org's free plan serves match state through an edge cache, so two requests minutes apart can return *different* states (e.g. one node still says `PAUSED` with the half-time score while another already says `FINISHED`). A live match can therefore look "stuck" — its `liveScore` frozen at an old snapshot — even though the cron fires every 5 min and returns `200 OK`, because the cron keeps receiving a stale cached snapshot. There is **no reliable free fix** (cache-busting query params / `Cache-Control` headers don't bypass their CDN). The final `result` always propagates eventually (so points are delayed, never lost); to un-stick a match immediately, run `npm run sync-results` or `npm run set-result -- <matchId> <h> <a>`. Note `runSync` excludes `status === 'finished'` matches from updates, so a stale `IN_PLAY`/`PAUSED` snapshot can **never** revert a finished match back to live.
