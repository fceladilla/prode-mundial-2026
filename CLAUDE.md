...
**The upstream `fetch` MUST stay `cache: 'no-store'`.** In Next.js App Router, `fetch()` is cached by the Data Cache by default. When `runSync` runs inside `GET /api/sync` on Vercel, an un-flagged fetch makes the cron evaluate a **stale cached snapshot** of the football-data API — a live match stays `upcoming` / the `liveScore` stays frozen — even though the cron fires every 5 min and returns `200 OK`. The CLI (`npm run sync-results`, plain Node) doesn't go through Next's cache, which is why running it locally always shows fresh data and "fixes" a stuck match. `fetchApiMatches` in `lib/sync.ts` passes `cache: 'no-store'` to force live data on every run; `dynamic = 'force-dynamic'` on the route alone did **not** propagate to the imported module's fetch. Symptom to watch for: prod `/api/sync` logs show a match as still `TIMED`/`PAUSED` while a direct API call (or `npm run diag-sync` vs the live API) shows it `IN_PLAY`/`FINISHED`. To un-stick a match immediately, run `npm run sync-results` or `npm run set-result -- <matchId> <h> <a>`. Note `runSync` excludes `status === 'finished'` matches from updates, so a stale `IN_PLAY`/`PAUSED` snapshot can **never** revert a finished match back to live.
