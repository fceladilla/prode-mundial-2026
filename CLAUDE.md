...
**Knockout stage**: matches are scored on the **90-minute (regulation) result only** — extra time and penalty shootouts are ignored, and there is no separate "who advances" pick (decided 2026-06-27). A tie decided on penalties is recorded and scored as a draw. The scoring engine (`computePoints`) is stage-agnostic; the rule lives in **how the sync picks the score**: `lib/sync.ts` reads `score.regularTime` (90' score, only populated by football-data.org when a match went to ET/penalties) and falls back to `score.fullTime` for matches that ended in regulation. **Never read `fullTime` directly** — in knockout it includes ET and the shootout (a 1-1 won 4-2 on pens arrives as 5-3). When loading a knockout result by hand with `set-result`, enter the **90-minute** score.
...
**Group standings** are served by `GET /api/standings` (`runtime = 'nodejs'`, `revalidate = 1800`), which proxies football-data.org's `/competitions/WC/standings`, caps the upstream call to once per 30 min (`next: { revalidate }` — the opposite of the live sync, where slow standings make caching desirable), and enriches each row with our team catalog (same Spanish-name + emoji-flag rendering as the rest of the app). Public, no auth. `app/clasificacion/page.tsx` fetches it client-side and renders one table per group (top 2 highlighted as qualifying). Standings come from the API, **not** computed from our `matches`, so FIFA tiebreakers are exact.
